...
function renderCanvas() {
  ...
  floorWalls.forEach(wall => {
    ...
    if (String(wall.id) === String(state.selectedWallId) || (state.selectedIds && state.selectedIds.map(String).includes(String(wall.id)))) {
    ...
    if (isHoriz) {
      re.style.cssText = 'left:-5px;top:50%;transform:translateY(-50%);cursor:ew-resize;';
    } else {
    ...
    if (String(wall.id) === String(state.selectedWallId)) {
    ...
    div.onmousedown = e => {
      ...
      state.selectedWallId = wall.id;
      if (e.shiftKey) {
        const wallStrId = String(wall.id);
        if (!state.selectedIds.map(String).includes(wallStrId)) {
          state.selectedIds.push(wallStrId);
        }
      } else {
        state.selectedIds = [String(wall.id)];
      }
      if (state.is3D) {
        renderAll();
        return;
  ...
  if (currentFloor) {
    currentFloor.blocks.forEach(block => {
      block.elements.forEach(el => {
        ...
        if (state.selectedIds && state.selectedIds.map(String).includes(String(el.id))) div.classList.add('selected');
        ...
        if (el.type === 'bridge' || el.name === 'Bridge') div.classList.add('bridge-element');
        ...
        div.onmousedown = e => {
          ...
          state.selectedWallId = null; // Clear selected wall
          if (state.is3D) {
            state.selectedIds = [String(el.id)];
            renderAll();
            e.stopPropagation();
            return;
          }
  ...
  for (const item of floorEls) {
    ...
    if (gate.axis === 'v') {
      if (Math.abs(cx - gate.x) < 15) {
        const y0 = gate.y0 !== undefined ? gate.y0 - 15 : Math.min(a.y, b.y);
        const y1 = gate.y1 !== undefined ? gate.y1 + 15 : Math.max(a.y + a.h, b.y + b.h);
        if (cy >= y0 && cy <= y1) {
    ...
    if (gate.axis === 'h') {
      if (Math.abs(cy - gate.y) < 15) {
        const x0 = gate.x0 !== undefined ? gate.x0 - 15 : Math.min(a.x, b.x);
        const x1 = gate.x1 !== undefined ? gate.x1 + 15 : Math.max(a.x + a.w, b.x + b.w);
        if (cx >= x0 && cx <= x1) {
          return { x: cx, y: cy, axis: gate.axis };
        }
      }
...
function drawPathArrows(path, edges, activeFloorId, animateLines) {
  ...
  let visualPath = [...path];
  // Commented out shifting/popping so the path line draws continuously from/to the room centers through the doors
  /*
  ...
  */
  ...
  function buildPolyline() {
    ...
    for (let i = 0; i < onFloor.length; i++) {
      ...
      // Door override: if a room is starting/ending at a door, draw the path line start/end at that door
      const elType = (el.type || '').toLowerCase();
      const isElPassage = ['corridor', 'door', 'elevator', 'staircase', 'hall', 'window', 'entry_exit', 'waypoint', 'bridge'].includes(elType) || elType.startsWith('corridor') || el.isStairs;
      if (!isElPassage) {
        if (i === 0 && i + 1 < onFloor.length) {
          const nextRec = getElById(onFloor[i + 1]);
          if (nextRec && ['door', 'window', 'entry_exit'].includes((nextRec.el.type || '').toLowerCase())) {
            cx = nextRec.el.x + nextRec.el.w / 2;
            cy = nextRec.el.y + nextRec.el.h / 2;
          }
        } else if (i === onFloor.length - 1 && i - 1 >= 0) {
          const prevRec = getElById(onFloor[i - 1]);
          if (prevRec && ['door', 'window', 'entry_exit'].includes((prevRec.el.type || '').toLowerCase())) {
            cx = prevRec.el.x + prevRec.el.w / 2;
            cy = prevRec.el.y + prevRec.el.h / 2;
          }
        }
      }
      ...
      if (i < onFloor.length - 1) {
        ...
        if (i === onFloor.length - 2) {
          ...
          const nElType = (nel.type || '').toLowerCase();
          const nIsElPassage = ['corridor', 'door', 'elevator', 'staircase', 'hall', 'window', 'entry_exit', 'waypoint', 'bridge'].includes(nElType) || nElType.startsWith('corridor') || nel.isStairs;
          if (!nIsElPassage) {
            if (['door', 'window', 'entry_exit'].includes((el.type || '').toLowerCase())) {
              ncx = el.x + el.w / 2;
              ncy = el.y + el.h / 2;
            }
          }
        ...
        // If it's an intermediate room (like a Hall), push its center so the path routes through it
        if (i < onFloor.length - 2) {
          const nElType = (nel.type || '').toLowerCase();
          const isIntermediatePassage = ['corridor', 'door', 'elevator', 'staircase', 'window', 'entry_exit', 'waypoint', 'bridge'].includes(nElType) || nElType.startsWith('corridor') || nel.isStairs;
          // Notice we omitted 'hall' from isIntermediatePassage, so halls behave like rooms and get center routed
          if (!isIntermediatePassage || nElType === 'hall') {
            pts.push({ x: nel.x + nel.w / 2, y: nel.y + nel.h / 2, elId: onFloor[i + 1], isGate: false });
          }
        }
      }
    }
  ...
  onFloor.forEach(elId => {
    ...
    // Door override: if a room is starting/ending at a door, position the start/end pins at that door
    const elType = (rec.el.type || '').toLowerCase();
    const isElPassage = ['corridor', 'door', 'elevator', 'staircase', 'hall', 'window', 'entry_exit', 'waypoint', 'bridge'].includes(elType) || elType.startsWith('corridor') || rec.el.isStairs;
    if (!isElPassage) {
      if (isFirst && path.length > 1) {
        const nextRec = getElById(path[1]);
        if (nextRec && ['door', 'window', 'entry_exit'].includes((nextRec.el.type || '').toLowerCase())) {
          cx = nextRec.el.x + nextRec.el.w / 2;
          cy = nextRec.el.y + nextRec.el.h / 2;
        }
      } else if (isLast && path.length > 1) {
        const prevRec = getElById(path[path.length - 2]);
        if (prevRec && ['door', 'window', 'entry_exit'].includes((prevRec.el.type || '').toLowerCase())) {
          cx = prevRec.el.x + prevRec.el.w / 2;
          cy = prevRec.el.y + prevRec.el.h / 2;
        }
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                drawPathArrows(window._pathState.path, window._pathState.edges, nextFloorId, true);
                applyPathHighlights(window._pathState.path, nextFloorId);
                buildFloorBar(window._pathState.path, window._pathState.edges, nextFloorId);
              }, 60);
            }, 300);
          }, 600);
          return;
...
function buildFloorBar(path, edges, activeFloorId) {
  ...
  floorsSeen.forEach(f => {
    ...
    btn.onclick = () => {
      ...
      renderAll();
      setTimeout(() => {
        drawPathArrows(window._pathState.path, window._pathState.edges, f.id, false);
        applyPathHighlights(window._pathState.path, f.id);
        buildFloorBar(window._pathState.path, window._pathState.edges, f.id);
      }, 60);
    };
    bar.appendChild(btn);
  });
...
function renderRestroomSign3D(div, el, gender) {
  const w = el.w;
  const h = el.h;
  // Create a flat 3D tile/pedestal for the sign so it's visible from the top view
  const d = el.height3D !== undefined ? el.height3D : 10;
  ...
  if (gender === 'male') {
    ...
    iconSvg = `<svg viewBox="0 0 40 40" style="width:75%;height:75%;">
      <circle cx="20" cy="10" r="4" fill="${iconColor}"/>
      <path d="M14,26 L14,19 C14,16 16,15 20,15 C24,15 26,16 26,19 L26,26" stroke="${iconColor}" stroke-width="2.5" fill="none"/>
      <line x1="17" y1="26" x2="17" y2="32" stroke="${iconColor}" stroke-width="2.5" stroke-linecap="round"/>
      <line x1="23" y1="26" x2="23" y2="32" stroke="${iconColor}" stroke-width="2.5" stroke-linecap="round"/>
    </svg>`;
    ...
    iconSvg = `<svg viewBox="0 0 40 40" style="width:75%;height:75%;">
      <circle cx="20" cy="10" r="4" fill="${iconColor}"/>
      <path d="M15,26 L17,17 C17,16 18,15 20,15 C22,15 23,16 23,17 L25,26 Z" fill="${iconColor}"/>
      <line x1="17" y1="26" x2="17" y2="32" stroke="${iconColor}" stroke-width="2.5" stroke-linecap="round"/>
      <line x1="23" y1="26" x2="23" y2="32" stroke="${iconColor}" stroke-width="2.5" stroke-linecap="round"/>
    </svg>`;
  } else {
    ...
    iconColor = '#ffffff';
    iconSvg = `<svg viewBox="0 0 40 40" style="width:75%;height:75%;">
      <circle cx="13" cy="12" r="3" fill="${iconColor}"/>
      <path d="M9,26 L9,19 C9,17 11,16 13,16 C15,16 17,17 17,19 L17,26" stroke="${iconColor}" stroke-width="2" fill="none"/>
      <circle cx="27" cy="12" r="3" fill="${iconColor}"/>
      <path d="M23,26 L24,18 C24,17 25,16 27,16 C29,16 30,17 30,18 L31,26 Z" fill="${iconColor}"/>
    </svg>`;
  }
  
  // The tile spans the entire width and height of the element
  // Add 12px offset to sit exactly on top of standard room floors (which are 12px thick)
  const floorOffset = 12;
  const tileBox = create3DBox(0, 0, floorOffset + d, w, h, d, bgColor);
  
  const svgContainer = document.createElement('div');
  svgContainer.style.cssText = 'position:absolute; inset:0; display:flex; align-items:center; justify-content:center;';
  svgContainer.innerHTML = iconSvg;
  
  tileBox.appendChild(svgContainer);
  div.appendChild(tileBox);
}
