...
function deleteSelected() {
  ...
  pushHistory();
  const toDelete = [...state.selectedIds].map(String);
  
  state.stairLinks = state.stairLinks.filter(lk => !toDelete.includes(String(lk.fromElId)) && !toDelete.includes(String(lk.toElId)));
  state.universalLinks = state.universalLinks.filter(lk => !toDelete.includes(String(lk.fromElId)) && !toDelete.includes(String(lk.toElId)));

  for (let b of (curFloor()?.blocks || [])) {
    b.elements = b.elements.filter(e => !toDelete.includes(String(e.id)));
  }

  state.walls = state.walls.filter(w => !toDelete.includes(String(w.id)));
...
function duplicateSelected() {
  ...
  els.forEach((el, index) => {
    ...
    clone.id = Date.now() + index + '_' + Math.random().toString(36).substr(2, 5);
...
function pasteElements() {
  ...
  state.clipboard.forEach((el, index) => {
    ...
    if (clone.isWall) {
      clone.x += 20;
      clone.y += 20;
      clone.floorId = state.currFloorId;
      delete clone.isWall;
      state.walls.push(clone);
    } else {
      clone.x += 20;
  ...
  state.selectedIds = newIds;
  showToast(state.clipboard.length + ' element(s) pasted');
  renderAll();
}
